...
- (void)keyBoardShow:(NSNotification *)notific
{
    ...
    _resignButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
    _resignButton.backgroundColor = [UIColor clearColor];
    [_resignButton addTarget:self action:@selector(resignButtonAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)_createSearchTextFiled
{
    
//    [_searchTextFiled endEditing:YES];
    
    _searchTextFiled = [[UITextField alloc] initWithFrame:CGRectMake(60, 5, kScreenWidth - 60 - 50 - 70, 30)];
    _searchTextFiled.delegate = self;
    _searchTextFiled.placeholder = @"  想买啥尽管搜";
    ...
    [_searchTextFiled addTarget:self action:@selector(textChange) forControlEvents:UIControlEventEditingChanged];
    
    [self.navigationController.navigationBar addSubview:_searchTextFiled];
    
    _searchButton = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 120, 5, 60, 30)];
...
- (void)searchButtonAction
{
    ...
    _backgroundScrollerView.hidden = NO;
    _giftButton.hidden = NO;
    _tacticButton.hidden = NO;
    _orderButton.hidden = NO;
    
    _keyword = _searchTextFiled.text;
...
- (void)textChange
{
    if (_searchTextFiled.text.length == 0) {
        _backgroundScrollerView.hidden = YES;
        _giftButton.hidden = YES;
        _tacticButton.hidden = YES;
        _orderButton.hidden = YES;
        _searchButton.enabled = NO;
    }
    else if (_searchTextFiled.text.length > 0)
    {
